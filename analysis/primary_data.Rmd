...
We calculated the effect size from a Cox model with fixed and random effects, by re-using the archived analysis code to reproduce the exact model chosen by the original authors (which included random effects accounting for the experimental design). We then extracted the difference in survival rate between females carrying either a female-limited (FLX) or control X chromosome (specifically the CFM control, i.e. a control X that had co-evolved with X balancer chromosomes), expressed as a log hazard ratio, which we transformed to Cohen's $d$. The effect size was made negative, since females with the female-limited X showed non-significantly worse longevity than controls.
...
These data come from the same Github repo ([link](https://github.com/KKLund-Hansen/FLXLongevityImmune)). The data give the mean bacterial load (in CFU) of the gram-negative _Lactococcus lactis_ in flies carrying three types of X chromosomes (female-evolved, control-evolved 'CFM', and wild type control). Higher bacterial load indicates lower resistance. 

#### Calculating effect size

We followed the code of the authors, which involved transforming the bacterial load data via $log(x+1)$ and removing one outlier, then running a linear mixed model with random effects accounting for the experimental design. We standardised the response variable (after the log transformation) to have mean 0, variance 1, such that the effect sizes were in units of Cohen's $d$.
...
These data come from the same Github repo ([link](https://github.com/KKLund-Hansen/FLXLongevityImmune)). The data give the number of surviving female flies 7 days post-treatment from experiments in which females were either naive, wounded, or infected with one of two bacteria (gram-negative _Lactococcus lactis_ or gram-positive _Pseudomonas entomophila_) in flies carrying three types of X chromosomes (female-evolved, control-evolved 'CFM', and wild type control). Higher bacterial load indicates lower resistance. The wounding treatment is a procedural control for the mode of infection.

#### Calculating effect size

We re-used the authors' code to run the same Cox survival model with random effects. We then calculated interaction contrasts using `emmeans`, to first find the difference in survival probability between the wounding control and the bacteria treatments, and second to compare this difference between the FLX and CFM treatments. Assuming removal of IaSC makes females healthier, we would predict a smaller worsening of survival in infected (vs wounded) flies in the FLX treatment. There is no statistically significant difference, however.
