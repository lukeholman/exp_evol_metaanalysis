...
Crucially, effect sizes have a sign (positive or negative, depending on whether mean 1 or mean 2 is the larger) as well as a magnitude. The sign is arbitrary, as it flips from positive depending on which mean is designated as mean 1 versus mean 2.

To ensure the effect size sign is meaningful and comparable across studies, I defined a positive effect size to indicate that the two means differ in the direction that one would predict if intralocus sexual conflict is present, and a negative effect to mean that the means differ in a direction opposite to this prediction. For example, for a study like Prasad et al. (2007) that removed selection on females and then measured mean male and female fitness relative to a control, we would predict male-evolved populations to have higher male fitness and lower female fitness if intralocus sexual conflict is present. So, a positive effect size should indicate higher male fitness but lower female fitness in the experimental group relative to the control. To clarify the meaning of each effect size, I provide a column called 'interpretation' in the final table of effect sizes, and document the more confusing/ambiguous decisions in this document below.
...
Load packages and define some custom functions:
...
However the study provides means only means, not a measure of error, which we need to compute effect size variance and confidence limits. To get a rough estimate of these, we assume that the error is comparable to similar male fitness assays carried out in Wong & Holman (2023; _Evolution_). We load that study's raw data, and find the average standard deviation in proportion of offspring sired relative to competitor males. We then use simulation to generate 10,000 possible datasets for Rice (1996) by generating random Normally distributed numbers using the means from Rice's Figure 1 and SD for Wong & Holman, and calculate 10,000 estimates of the standardised effect size Cohen's d comparing the two treatments. From these 10,000 estimates we also obtain the effect size variance and 95% CIs. 
...
estimated_SD <- read_csv("input_data/male_fitness_raw_data_wong_and_holman.csv", show_col_types = FALSE) %>% 
  mutate(fitness = `Early male focal` / (`Early male focal` + `Early male rival`)) %>% 
  group_by(Line) %>% 
...
bootstrapped_effect_sizes <- lapply(1:1000, function(i){
  ...
  possible_dataset <- map_df(1:8, ~ tibble(
    ...
    mutate(line = paste(treatment, replicate, sep = "_"),
           fitness = as.numeric(scale(fitness)))
  
  as.numeric(summary(lm(fitness ~ treatment + line, data = possible_dataset))$coefficients[2,1:2])
}) %>% 
  do.call("rbind", .) %>% 
  as.data.frame() %>% as_tibble() %>% 
  mutate(V2 = V2 ^ 2) %>% # Square the SE in the difference in means, to get the variance
...
This paper measured the abundance of dozens of hydrocarbons present on the body surface of both sexes, in the male-limited evolution treatment or the control lines. The authors analysed each hydrocarbon with its own univariate test, and largely found no differences between treatments. Since there are many hydrocarbons, we have not included them all here: we focus on a single male-unique hydrocarbon, 7-C_{23:1}, which is known to function as a male sex pheromone (e.g. Grillet et al., 2006; https://doi.org/10.1098/rspb.2005.3332). The means and standard errors in the code chunk below come from row M7 of Table 1, and the sample sizes are inferred from the methods (30 males per selection line, 4 selection lines per treatment = 120 males per treatment assuming no missing samples). The positive (non-significant) effect size implies more sex pheromones in the ML males.
...
Abbott, J. K., Chippindale, A. K., & Morrow, E. H. (2020). The microevolutionary response to male-limited X-chromosome evolution in Drosophila melanogaster reflects macroevolutionary patterns. _Journal of Evolutionary Biology_, 33, 738–750.
...
```{r}
SF.data <- read_csv("input_data/lundhansen_github_data_FLXevoexp-master/StandardFitnessGeneration39-41 /StandardFitnessG3941.csv")
...
```{r}
dev_time <- read_csv("input_data/lundhansen_github_data_FLXevoexp-master/DevelopmentTimeGeneration/DevelopmentTimeG43.csv")
...
# CFM females take non-sig longer to develop than FLX (this is the prediction, since shorter development is a feminine trait)
...
```{r}
loco <- read_csv("input_data/lundhansen_github_data_FLXevoexp-master/LocomotionAssayGeneration123/LocoAssayG123.csv")
...
```{r}
thorax <- read_csv("input_data/lundhansen_github_data_FLXevoexp-master/ThoraxSizeGeneration72/ThoraxSizeG72.csv")
...
```{r}
# Here, I could not reproduce the original analysis as the archived code doesn't work. I did get this file from the authors, and here I do a simpler GLMM that replicates the original more or less, but does not include an offset term.

# tally up the total observed maters and non-maters for each sex and population
DsRed_Data <-  read_csv("input_data/grieshop_2025_DsRed_Data_Formatted.csv")  %>% 
  group_by(Population, Sex, Marker) %>% 
  summarise(n_mated = sum(`Mating Status` == "Mated"),
            n_non_mated = sum(`Mating Status` == "Non-Mated")) %>% 
  mutate(Marker = factor(Marker, c("Red", "Non-Red")))

# binomial GLMM that finds the sex-specific effects of the Red marker (vs non-red) on mating propensity
# This model assumes that all mated and non-mated flies were counted, which I think is not the case, but this should add random error rather than biasing the effect size estimates 
model <- glmer(cbind(n_mated, n_non_mated) ~ Sex * Marker + (1 | Population), data = DsRed_Data, family = binomial)

# calculate effect of marker on mating rate (as a log odds ratio) separately for each sex
LOR_effects <- contrast(emmeans(model, ~ Marker | Sex), method = "pairwise") %>% 
  as_tibble() %>% 
  mutate(estimate = c(-1,1) * estimate) %>% # change sign of the effect size for females
  rename(logOR = estimate) %>% 
  mutate(Estimate = logOR * sqrt(3) / pi,
         Var = (SE * sqrt(3) / pi)^2,
         Lower_95_CI = (logOR - SE * 1.96) * sqrt(3) / pi,
...
tibble(Study = "Grieshop et al. (2025) Molec. Biol. Evol. 42 msaf099.",
       ...
       Generations = 103,
       Organism = "Drosophila melanogaster",
       Interpretation = c("A positive effect would mean that females carrying primarily male-evolved copies of chromosome 2 mate less often, compared to females with primarily female-evolved copies of chromosome 2 (there is no difference, though).", 
...
emmeans(modfitCS, ~ Treatment | sex_role) %>% plot()
...
Martinossi_Allibert %>% 
  group_by(treatment, environment, sex) %>% 
  summarise(mean = mean(LRS),
            se = sd(LRS) / sqrt(n()),
            ymin = mean-se,
            ymax = mean+se) %>% 
  ggplot(aes(environment, mean, colour = treatment, group = paste(treatment, sex))) +
