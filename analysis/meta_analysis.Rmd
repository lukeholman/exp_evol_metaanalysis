...
plot_data$Short_name[plot_data$Short_name == "Melo-Gavin et al. (2026)"] <- "**Symmetrical sex-limited autosomes**<br>Melo-Gavin et al. (2026)"
...
**Figure 1**: Forest plot showing the estimated effect sizes (Cohen's _d_ \(\pm \) 95% CIs). The y-axis indicates the trait being measured while horizontal grey bands show the `r nrow(bands)-1` independent experimental evolution projects (projects are measured in one or more publications, named on the right). Positive effect sizes indicate evolutionary change in the direction predicted under IaSC, while negative effects indicate evolution in the opposite direction. Point colour highlights experiments using sex-limited inheritance of autosomal (blue) or X (red) genomic regions, or sex-specific relaxed selection (green). The mean effect size from meta-analysis is shown in black.

## Plotting effect of moderator `Generations`

On the right of x-axis (high generation number), this result is driven by a couple of studies, Lund-Hansen et al. (2020) and Manat et al. (2021), which both relate to the same set of experimental evolution lines, and which both focus on female-limited evolution of X chromosomes (uniquely in the full dataset). I also calculated the effect sizes from the raw data for these studies, which avoids falsely inflated the effect sizes (as for some studies where only the overall means were available, incurring pseudoreplication). To the left of the x-axis (short generation number), the trend is driven by a couple of older studies I calculated the effect sizes from overall means, possibly inflating them.

Therefore, I don't feel there is good evidence for a true negative effect of `Generations`, since this result could easily result from confounding.
