...
On the right of x-axis (high generation number), this result is driven by a couple of studies, Lund-Hansen et al. (2020) and Manat et al. (2021), which both relate to the same set of experimental evolution lines, and which both focus on female-limited evolution of X chromosomes (uniquely in the full dataset). It's possible that making the X female-limited results in less evolution than other designs (e.g. male-limited X or autosomes), e.g. because the X is already present in females 66% of the time, though without replication this conclusion is tentative. Furthermore, generation time is confounded with the method that we used to calculate effect size. All the high-generation studies ( Lund-Hansen et al., Manat et al., and Grieshop et al.) had accessible raw data while many shorter-generation ones were older studies that predate the practice of data archiving, necessitating calculation of effect size from summary statistics in the latter (which potentially inflates the effect size). 
