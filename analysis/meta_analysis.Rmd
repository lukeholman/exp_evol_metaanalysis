...
meta_analysis_data <- list.files("effect_sizes/", full.names = T) %>% 
  ...
  mutate(
    ...
    Sex = replace(Sex, grepl("male-limited lines", Trait), "Male"),
    Sex = replace(Sex, grepl("female-limited lines", Trait), "Female")) %>% 
  select(-first_word)
```
...
This table is scrollable, searchable, and can be exported as a `.csv` file. You can also sort by column, or hide columns. The table lists `r n_effects` effect sizes (and their associated variance and 95% CIs) from `r n_expts` independent experimental evolution projects, which were published across `r n_pubs` journal articles. The `Trait` column gives the name of the trait being measured, `Generations` gives the number of generations of selection before measuring this trait, `Interpretation` gives a plain English description of what the effect size means, `Origin_of_lines` column gives the reference that first created the lines, the two `Experiment_type` columns are two ways of categorising the type of experiment, and `Sex` gives the sex of the individuals being measured
...
## Testing for robustness by removing 3 studies

The literature review highlighted possible confounding effects in Rice (1996), Prasad et al. (2007), and Abbott et al. (2013) that might mean that improvements in fitness were due to experimental conditions, rather than allele frequency change at sexually antagonistic loci. We here re-run the meta-analysis excluding these three studies to verify that the mean effect size remains positive.

```{r}
filtered_meta_analysis_data <- meta_analysis_data %>% 
  filter(!(Origin_of_lines %in% 
             c("Prasad et al. (2007)", 
               "Rice (1996)", 
               "Abbott et al. (2013)")))

meta_analysis_strict <- rma.mv(
  yi = Estimate,
  V = Var,
  random = ~ 1 | Origin_of_lines/Study,
  data = filtered_meta_analysis_data,
  method = "REML"
)

summary(meta_analysis_strict)
```
...
The top model includes the moderator `Generations`, and it is ahead of second-best model (which also includes `Sex`) by more than 2 AICc, suggesting that `Generations` explains substantial variation in the data but the other moderators do not.
...
gen_ma <- fit_ma("~ Generations")

mean_effect_gen <- paste0(pretty3(as.numeric(gen_ma$b[1, 1])), " (",
                          pretty3(as.numeric(gen_ma$ci.lb)[1]), "-",
                          pretty3(as.numeric(gen_ma$ci.ub)[1]), ")", collapse = "")
```


Below are the results of the top model (ranked by AICc) with one or more moderators, showing the significant negative effect of the moderator `Generations` on effect size (note: `Generations` was scaled to have mean 0, SD 1, prior to analysis). This is a counter-intuitive result since we might expect greater effects after more generations of selection, but it is not very informative since `Generations` is confounded with other factors (e.g. species, type of experimental design, type of trait being measured, and whether I used the raw data or summary statistics to calculate effect size). The grand mean effect size (predicted here for the average value of `Generations`, which is `r mean_gen`) is `r mean_effect_gen`, which is much the same as the previous meta-analysis that did not adjust for the effect of `Generations`.
...
## Forest plot of effect sizes (Figure 2)

```{r fig.height = 7.96, fig.width=6, message=F, warning=F}
...
plot_data$Short_name[plot_data$Short_name == "Nordén et al. (2020);\nCīrulis et al. (2026)"] <- "**Reciprocal sex-limited markers**<br>Nordén *et al* (2020)<br>Cīrulis *et al*. (2026)"
...
plot_data$Short_name[plot_data$Short_name == "Lund-Hansen et al. (2020);\nManat et al. (2021);\nLund-Hansen et al. (2022);\nLund-Hansen et al. (this issue);\nManat et al. (this issue)"] <- "**Female-limited X**<br>Lund-Hansen *et al*. (2020)<br>Manat *et al*. (2021)<br>Lund-Hansen *et al*. (2022)<br>Lund-Hansen *et al*. (this issue)<br>Manat *et al*. (this issue)"
...
plot_data$Short_name[plot_data$Short_name == "Bonel et al. (2018)"] <- "**Relaxed selection on both sex functions**<br>Bonel *et al*. (2018)"
...
ggsave(meta_plot, height=7.96, width=6, filename = "figure_2.svg")
meta_plot
```

**Figure 2**: Forest plot showing the estimated effect sizes (Cohen's _d_ \(\pm \) 95% CIs). The y-axis indicates the trait being measured while horizontal grey bands show the `r nrow(bands)-1` independent experimental evolution projects (projects are reported in one or more publications, named on the right). Positive effect sizes indicate evolutionary change in the direction predicted under IaSC, while negative effects indicate evolution in the opposite direction. Point colour highlights experiments using sex-limited inheritance of autosomal (blue) or X (red) genomic regions, or sex-specific relaxed selection (green). The mean effect size from meta-analysis is shown in black.

## Plotting effect of moderator `Generations`

In short, it seems likely the negative effect of `Generations` on effect size results from confounding (i.e. the true causes are correlates of `Generations`, not that variable itself).

On the right of x-axis (high generation number), this result is driven by Lund-Hansen et al. (2020) and multiple studies based on the lines from it, which all relate to the same set of experimental evolution lines, and which both focus on female-limited evolution of X chromosomes (uniquely in the full dataset). It's possible that making the X female-limited results in less evolution than other chromosomes, or male-limited evolution designs, though without replication this conclusion is tentative. Furthermore, generation time is confounded with the method that we used to calculate effect size. All the high-generation studies (e.g.  Lund-Hansen et al. and its descendant studies, and Grieshop et al.) had accessible raw data while many shorter-generation ones were older studies that predate the practice of data archiving, necessitating calculation of effect size from summary statistics (which potentially inflates the effect size). 
